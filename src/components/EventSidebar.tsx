...
import { getEventWithRelations } from "@/lib/dataHelpers";
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {events.map((event) => {
          const eventWithRelations = getEventWithRelations(event);
          return (
            <div
              key={event.id}
            ...
            >
              ...
              <div className="flex w-full flex-col">
                ...
                <div className="text-muted-foreground flex gap-2 text-sm">
                  <p>
                    {format(parseISO(String(event.startDate)), "hh:mm a")}{" "}
                    {format(parseISO(String(event.startDate)), "do MMM yyyy")}
                  </p>
                </div>
                <p className="text-muted-foreground text-sm">{eventWithRelations.location.name}</p>
              </div>
            </div>
          );
        })}
